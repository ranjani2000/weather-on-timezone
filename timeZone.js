...
let timeZonesList = JSON.parse(fs.readFileSync("./timeZonesList.json", "utf8"));

// Maintain All methods and data inside WeatherForTimeZone class
class WeatherForTimeZones {
  constructor() {
    this.timeZones = {};
    for (let city in timeZonesList) {
      this.timeZones[timeZonesList[city].cityName] = this.getTimeForZone({
        timeZone: timeZonesList[city].timeZone,
      });
    }
